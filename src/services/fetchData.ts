...
export async function fetchAllCharacters(searchParams: string): Promise<RickAndMortyApiResponse> {
  let allCharacters: RickAndMortyApiResponse = {
...
export async function fetchCharactersByPage(searchParams: string): Promise<RickAndMortyApiResponse> {
  const { data }: AxiosResponse<RickAndMortyApiResponse> = await axios.get(
